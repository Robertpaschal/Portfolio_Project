...
def read_document(document_id: int, db:Session = Depends(database.get_db), user: models.User = Depends(get_current_user)):
    """Queries the databse and returns the Document(s) associated with the user using the provided ID"""
...
def list_documents(skip: int = 0, limit: int = 10, db: Session = Depends(database.get_db), user: models.User = Depends(get_current_user)):
    """Returns a list of documents for the current user, with pagination."""
    documents = db.query(models.Document).filter(models.Document.owner_id == user.id).offset(skip).limit(limit).all()
    return documents

@router.post("/generate")
def content_generation(prompt_request: schemas.PromptRequest, db:Session = Depends(database.get_db), user: models.User = Depends(get_current_user)):
    """This route uses the Gemini API to generate, summarize contents based on  users input.
    ...
    try:
        ...
        document_data = schemas.DocumentCreate(title=prompt_request.title, content=response)
