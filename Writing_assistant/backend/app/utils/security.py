...
def create_access_token(data: dict, expires_delta: timedelta = None):
    """Creates a token using a randomly generated secret key, encodes it as a JSON Web Token"""
    to_encode = data.copy()
    if expires_delta:
...
def get_current_user(token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)):
    """using the verified token, this function checks for the assoicated user in the database and returns it"""
